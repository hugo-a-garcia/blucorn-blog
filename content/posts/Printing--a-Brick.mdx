...
The design model created by Ootus serves as the baseline. It is well constructed given the dimensions. He does not provide a print using the dimensions. Here we print a 2x2 plate to test the basic dimension using the least amount of material. We use a 4x2 Lego brick to test connectivity with the plate.

The connectivity to the studs of the Lego brick is tight but acceptable. The connectivity to the bottom of the Lego brick is not possible due to the small diameter of the studs. The stud diameter in the design model is 4.80 mm but the product model is 4.70. The difference is caused by accumulated rounding and systematic errors from  the slicer and the printer operation. The errors can be compensated by adding small O.xx mm to the diameter of the stud in the design model. It is very important to note that the measures stud diameter of a Lego block is 4.88 mm. Correcting the diameter of the stud is the focus of the proceeding iteration.

### Iteration 2.0 - New parametric model in Fusion 360

A new design model was made which uses the LDU as a base unit and with parameters for almost every dimension of the part in oder to facilitate the printing of a Lego part.
